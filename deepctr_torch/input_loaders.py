...
class MultiShardsPairDataset(Dataset):
    def __init__(self, shard_paths, header, feat_cols, query_maxlen, doc_maxlen, phase='train', oov=-1):
        self.shard_paths = shard_paths
        self.header = header
        self.phase = phase
        self.feat_cols = feat_cols
    ...
    def _get_cur_dataset(self, shard_idx):
        if shard_idx != self.cur_shard_idx:
            ...
            for row in df:
                ...
                dataset.append((np.asarray(feat), 1.0))
            dataset_size = len(dataset)
            for i in range(dataset_size):
                anchor_idx = random.randint(0, dataset_size)
                neg_idx = random.randint(0, dataset_size)
                # if self.phase != 'train':
                #     newfeat = np.concatenate([dataset[anchor_idx][:self.query_maxlen],
                #                              dataset[neg_idx][self.query_maxlen:]])
                #     dataset.append(newfeat)
                # else:
                newfeat = np.concatenate([dataset[anchor_idx][0][:self.query_maxlen],
                                         dataset[neg_idx][0][self.query_maxlen:]])
                dataset.append((newfeat, 0.0))
